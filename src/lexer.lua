...
local DELIMETERS = {' ', '\n', '	'}
local IDENTIFIERS = {'{', '}', '#', '=', '\'', '"', ';', ':', '.', '$', '@', '[', ']', '!'}
local TOKEN_TYPE = {WORD = 1, COMMENT = 2, IDENTIFIER = 3, NUMBER  = 4, STRING = 5}

local function icontains(tabl, object)
	for i = 1, #tabl do
		if tabl[i] == object then
			return true
		end
	end
	return false
...
local function find(tabl, value, pos)
	for i = pos, #tabl do
		if tabl[i].value == value then
			return i
		end
	end
	return -1
